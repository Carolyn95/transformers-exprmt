...
#target: named entity recogniton: Miss MISC Universe MISC
# He O said O the O lifestyle O associated O with O being O Miss B-MISC Universe I-MISC could O make O routine O exercise O difficult O . O
# 20201210
import pdb
import pandas as pd
import os
...
def convertConllToT5(file_path):
  ...
  # print(file_dir)
  if 'dev' in file_path:
    save_dir = file_pdir + '/processed_conll_TaskLabel/val/'
  elif 'test' in file_path:
    save_dir = file_pdir + '/processed_conll_TaskLabel/test/'
  elif 'train' in file_path:
    save_dir = file_pdir + '/processed_conll_TaskLabel/train/'
  else:
    raise ValueError(
  ...
  for d in data:
    ...
    temp_target = [
        'named entity recogniton: ' + t.split(' ')[0] + ' ' +
        t.split(' ')[-1][2:] for t in d.splitlines() if t.split(' ')[-1] != 'O'
    ]
