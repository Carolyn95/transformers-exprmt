...
if __name__ == "__main__":
  ...
  paths = ['t5_10pctconll_ner',
          't5_base_10pctconll_ner']  # [ckpt_path, model_path]
  ...
  loader = DataLoader(dataset, batch_size=16, sampler=splr, shuffle=False, drop_last=True)
  ...
  for i in range(len(loader)):
    ...
    uniq_labels = ['B-LOC', 'B-ORG', 'B-MISC', 'B-PER', 'I-LOC', 'I-ORG', 'I-MISC', 'I-PER', 'O']
    # pdb.set_trace()
    for idx, (one_dec, one_target) in enumerate(zip(dec, targets)):
      ...
      # dec[idx] = one_dec_
      min_len = len(one_dec) if len(one_dec) <= len(one_target) else len(one_target)
